...
class OralClassifierModule(LightningModule):
    ...
    def __init__(self, model, weights, num_classes, lr=10e-3, max_epochs=100, features_size=2048, frozen_layers=-1):
        super().__init__()
        ...
        # Set the classifier layer with 2048 feature output
        self._set_model_classifier(weights_cls, num_classes, features_size)

        # Freeze layers if needed
        for idx, param in enumerate(self.model.parameters()):
            param.requires_grad = False if idx < frozen_layers else True

        # Attach additional classifier head
    ...
    def _set_model_classifier(self, weights_cls, num_classes, features_size): 
        ...
        if "ConvNeXt" in weights_cls:
            ...
            self.model.head = torch.nn.Sequential(
                torch.nn.Dropout(0.5),
                torch.nn.Linear(in_features, features_size)
            )
    ...
    def remove_network_end(self):
        # Strip off the final classifier used for prediction
        if isinstance(self.model, torch.nn.Sequential):
            self.model = self.model[0]  # Only keep the base model
