...
def get_transformations(cfg):
    ...
    train_img_tranform = torchvision.transforms.Compose([
        ...
        torchvision.transforms.ToTensor(),  # this must be AFTER all PIL transforms
    ])

    # Apply common transform to val/test/infer
    val_img_tranform = common_transform
    test_img_tranform = common_transform
    img_tranform = common_transform
...
def get_last_version(path):
    ...
    if not folders:
        print(f"[Info] No versioned folders found in '{path}', returning default version_0.")
        return "version_0"
...
def get_loggers(cfg):
    """
    Returns a TensorBoardLogger based on the hydra config
    cfg: hydra config with fields like cfg.logging.save_dir, name, version
    """
